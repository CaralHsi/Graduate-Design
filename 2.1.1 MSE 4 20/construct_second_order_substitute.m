function [NOE,output_args, sample_point_temp, sample_value_temp] = construct_second_order_substitute( HDMR, f_append, sample_point_whole,sample_value,x_0,f_0,objective_function,NOE,K,lower_bound, upper_bound, second_order_count, sample_point, x_0_old)
...
for kk = 1:l
    sample_value_temp(kk) = objective_function(sample(kk, :)) - HDMR(sample_point_temp(kk, 1:end - 1)) - f_append.func(sample_point_temp(kk,end)) - prefunctotal(sample_point_temp(kk, 1:end - 1));
end
% sample_point_temp = [sample; sample_point_temp];
% sample_value_temp = [sample_value_temp; sample_point_value];
% sample_point_temp = sample_point_temp(:,K(1:second_order_count + 1));
count = 0;
% sample_point_temp = sample_point_temp(:,choose);
for loop = 1:100000000000%8*(second_order_count)
...
loop
pre.prefunc = prefunctotal;
pre.func = @(x)pre.func(x) + pre.prefunc(x);
output_args = pre;
end