...
for i = 1:dimension_number
    ...
    test_value = objective_function(test_point) - HDMR(test_point(K(1:dimension_number))) + pre0.func(test_point(K(1:dimension_number)));
    NOE = NOE + 1;
    count = 0;
    while (count < 2)
...
%         HDMR = @(x)HDMR(x) - pre0.func(x) + pre.func(x); % no more needed
%         pre0 = pre;
        t = rand();
        test_point = t.*(upper_bound(K(i)) - lower_bound(K(i))) + lower_bound(K(i));
        test_point = point1DtoND(test_point, x_0_new, K(i));
        test_value = objective_function(test_point) - HDMR(test_point(K(1:dimension_number))) + pre0.func(test_point(K(1:dimension_number)));  
        if (detect2(test_value, pre.func(test_point(K(1:dimension_number))), objective_function(test_point)) == 1)
            count = count + 1;
        else
            count = 0;
        end
