...
upper_bound_current = upper_bound; % initialize the current bound
...
[x_0_new, f_0_predict] = ga(HDMR,dimension,[],[],[],[],lower_bound_current,upper_bound_current,[], ga_opts); % optimization
...
[x_0_dimension, f_0_predict] = ga(HDMR,1,[],[],[],[],lower_bound_current(K(1)),upper_bound_current(K(1)), [], ga_opts);  % optimize HDMR functions and get new x_0 and f_0
...
%         K(dimension_number)  = selected_dimension;
%     end
    [S,V] = sensitivityAnalyse(dimension, HDMR, lower_bound_current, upper_bound_current, K);
    selected_dimension = roullete(S);
    K(dimension_number) = selected_dimension
    ...
    for j = 1:(dimension_number - 1)
        ...
        [NOE,f{i,second_order_count}] = construct_second_order(f,[j dimension_number],sample_point,sample_value,x_0,f_0,objective_function,NOE,K);
    end
    
    %% function constructed in first-dimension optimazation
    HDMR = @(x)HDMR_function_2(f,f_0,x,dimension_number,second_order_count,choose); % HDMR function
    x_0_old = x_0_new; f_0_old = f_0_new; % update old x_0 and f_0
    [x_0_dimension,  f_0_predict] = ga(HDMR,dimension_number,[],[],[],[],lower_bound_current(K(1:dimension_number)),upper_bound_current(K(1:dimension_number)), [], ga_opts);  % optimize HDMR functions and get new x_0 and f_0
